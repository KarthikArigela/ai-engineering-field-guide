...
A note on this update: I re-extracted the whole Feb-Jul corpus in August with a rewritten prompt after finding the original extraction had drifted mid-series (the model silently changed partway through, which showed up as spurious swings in the numbers - see [the appendix](trends-appendix.md#data-and-methodology) and [_internal/eval/](_internal/eval/)). All eight months here now come from one consistent, validated extraction. Some of what follows reads differently from earlier versions of this analysis as a result - a few "trends" I reported before turned out to be measurement artifacts, not market movement. I've noted those below rather than quietly dropping them.

## Since February: what actually moved

Putting the first scrape (Feb 4) next to the latest (Aug 25) directly:

- ai-first share: 71.5% -> 70.2%, essentially unchanged. The market has held a steady 68-71% ai-first mix the entire eight months - there was no consolidation trend in either direction, despite what I reported earlier.
...
- `Prompt Engineering` (26.1% -> 39.0%) and `RAG` (32.8% -> 43.2%) are the fastest-moving named skills, not `SQL` as I reported before - that read was a measurement artifact from the extractor switch (more on this below).
- Inference/serving work within ai-first roles roughly halved (19.8% -> 9.9%), and `Model Deployment` specifically fell from 18.3% to 6.9%. The dedicated AI-infra role is even less evident now than the six-scrape data suggested.
...
- Average skill breadth per posting did not trend upward the way I previously reported - it bounced in an 18-23 skill range all year rather than climbing steadily. "The posting is getting fatter" was mostly noise.
...
Within ai-first jobs, the move from model-training toward integration and orchestration is the one finding that gets stronger, not weaker, once the extraction is consistent:
...
The agent-builder archetype split into two distinct clusters since the six-scrape version of this analysis - one built around prompt engineering and provider APIs, one built around agent frameworks with little to no retrieval. Meanwhile the old DevOps/infra and full-stack clusters merged into one. Read together with the FDE numbers below, agent-building keeps fragmenting into more specific sub-roles rather than consolidating into one job description.
...
I previously reported `SQL` as the single fastest-rising skill, at +3.59 pp per scrape. On the corrected, consistently-extracted data that isn't true - `SQL` moved from 11.1% to 15.8%, a real but modest gain, nowhere near the top of the list. The earlier number was an artifact of the mid-series extractor change, not a market signal. This is exactly the kind of mistake re-extracting was meant to catch.
...
`MCP` and `Fine-Tuning` both now sit closer to the application stack than the training one - a shift from the earlier read, where fine-tuning pulled more heavily toward MLOps and infra.
...
I previously reported that postings were accumulating requirements over time (14.9 to 19.5 average skills). On the corrected data that isn't right either: skill breadth bounced in an 18-23 skill range all eight months without a clear upward trend. GenAI-specific skill count did tick up modestly, 4.6 to 5.5 per posting, but the broader "the role keeps demanding more" story doesn't hold once the extraction is consistent.
